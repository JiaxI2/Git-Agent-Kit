...
func TestSaveAndLoadJSONFromRepo(t *testing.T) {
	repo := t.TempDir()
...
func TestLoadFromRepoExplainsInitializationAndInvalidJSON(t *testing.T) {
	repo := t.TempDir()
	if _, err := LoadFromRepo(repo); err == nil || !strings.Contains(err.Error(), "gia init --repo") {
		t.Fatalf("missing config error lacks recovery guidance: %v", err)
	}
	...
	if _, err := LoadFromRepo(repo); err == nil || !strings.Contains(err.Error(), "parse ") {
